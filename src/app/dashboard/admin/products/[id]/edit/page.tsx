  ...
  useEffect(() => {
    async function loadProduct() {
      ...
      try {
        ...
        const groupedVariations = groupProductVariants(product);
        ...
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-2 text-slate-600 dark:text-slate-400">
              Update product information, images, variations, sizes, prices, and
              stock.
            </p>
          </div>
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                These images are used on product cards and product previews.
              </p>
            </div>
          ...
          <div className="mt-6 space-y-6">
            {variations.map((variation, variationIndex) => {
              ...
              return (
                <div
                  key={variationIndex}
                  className="rounded-[2rem] border border-slate-200 p-5 dark:border-slate-800"
                >
        ...
        <DialogContent className="max-h-[92vh] w-[95vw] max-w-none overflow-hidden rounded-[2rem] border-slate-200 p-0 dark:border-slate-800 sm:max-w-6xl xl:max-w-7xl">
          <DialogHeader className="border-b border-slate-200 bg-slate-50 px-6 py-5 dark:border-slate-800 dark:bg-slate-950">
            ...
            <DialogDescription className="text-slate-600 dark:text-slate-400">
              Review all product changes before updating this product.
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-[1.5rem] border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3">
                    <SummaryBox label="Total Stock" value={String(totalStock)} />
                    <SummaryBox
                      label="Variations"
          ...
          <DialogFooter className="flex-col gap-3 border-t border-slate-200 bg-slate-50 px-6 py-4 mb-4 dark:border-slate-800 dark:bg-slate-950 sm:flex-row sm:items-center sm:justify-between">
            <Button
              type="button"
              variant="outline"
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              >
                {isSubmitting ? "Updating Product..." : "Confirm & Update"}
              </Button>
            </div>
          </DialogFooter>
        </DialogContent>
...
function groupProductVariants(product: Product): ProductVariationInput[] {
  ...
  const grouped = new Map<string, ProductVariationInput>();

  product.variants.forEach((variant) => {
    const colorName = variant.color?.name || "Default";
    const hexCode = variant.color?.hexCode || "#000000";
    const groupKey = `${colorName}-${hexCode}`;

    const existingGroup = grouped.get(groupKey);

    const sizeRow: ProductSizeInput = {
      size: variant.size || "",
      basePrice: String(variant.price || product.basePrice || ""),
      discountPrice:
        product.discountPrice && product.discountPrice < variant.price
    ...
    if (existingGroup) {
      existingGroup.sizes.push(sizeRow);

      const existingImageUrls = new Set(
        existingGroup.images.map((image) => image.url)
      );

      variantImages.forEach((image) => {
        if (image.url && !existingImageUrls.has(image.url)) {
